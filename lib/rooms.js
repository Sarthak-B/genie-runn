...
function view(r, role, now) {
  ...
  return {
    protocol: 'realtime-3', room: r.id, role, phase: r.phase, round: r.round,
    ...
    hitBy: r.hit_by ?? null, reason: r.reason, otherPeer: r[other + '_peer'] ?? null,
...
export async function roomAction(b, redis, now = Date.now()) {
  ...
  if (b.action === 'create') {
    const r = {
      id: token(), host_token: token(), guest_token: null, phase: 'waiting', round: 0, seed: seed(),
  ...
  for (let attempt = 0; attempt < 12; attempt++) {
    ...
    if (b.action === 'join') {
      ...
      if (!['sync', 'ready', 'pause', 'resume', 'end', 'leave', 'signal'].includes(b.action)) return reply('Unknown room action.', 400);
      r[role + '_seen'] = now;
      ...
      if (b.action === 'end') {
        ...
        // A collision sent before a concurrent pause remains authoritative when it arrives.
        if (sameRound && ['running', 'paused'].includes(r.phase)) {
          r.phase = 'over'; r.hit = b.hit; r.hit_by = role; r.host_ready = 0; r.guest_ready = 0; r.reason = '';
        }
      }
