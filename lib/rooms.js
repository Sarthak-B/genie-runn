...
function view(r,role,now){
 return {room:r.id,role,phase:r.phase,round:r.round,seed:r.seed,startAt:r.start_at,serverTime:now,selfSeq:r[role+'_seq'],worldTime:r.host_state_at??null,hostReady:!!r.host_ready,guestReady:!!r.guest_ready,joined:!!r.guest_token,hostSeen:r.host_seen,guestSeen:r.guest_seen,hit:r.hit,reason:r.reason,other:JSON.parse((role==='host'?r.guest_state:r.host_state)||'null'),self:JSON.parse((role==='host'?r.host_state:r.guest_state)||'null'),world:JSON.parse(r.host_state||'null')};
}
